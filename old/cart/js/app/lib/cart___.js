...
//TODO: if prices forming is complicated, get data from server
(function (factory) {
 'use strict';
 ...
 }else
 {
  if('SiteManager' in window)
  {
   ...
   SiteManager.lib['Cart']=factory(jQuery,SiteManager);
  }
 }
}(function($,mgr){
 function Cart(opts){
 ...
 $.extend(Cart.prototype,{
  ...
  onstorage:function(){
   ...
   mgr.helpers.win.on('storage',function(e){
    if(e.originalEvent.newValue&&JSON.parse(e.originalEvent.newValue)[self.options.storageFix]==self.props.unique)
     return;
  ...
  reconstruct:function(storeFlag){
   ...
   if(!s&&!(self.options.saved&&self.options.saved.data.length)||s&&!JSON.parse(s).data.length)
   {
    self.emptied(true);
  ...
  storeData:function(data){
   ...
   for(var x in data)
    s.data.push(data[x].data);

   s[self.options.storageFix]=self.props.unique;

...
(function (factory) {
 ...
 $.extend(Carousel.prototype,{
  destroy:function(){
   ...
   self.props.block.off('webkitTransitionEnd'+n+' transitionend'+n+' mouseenter'+n+' mouseleave'+n+' mousewheel'+n+' swipe'+n+' touchstart'+n);
  ...
  blockTouchStart:function(e){
   ...
   mgr.helpers.doc.on('touchmove'+self.options.namespace,function(e){
    ...
    if(self.props.touch&&Math.abs(delta)>self.options.touchThreshold&&Math.abs(delta)>2*Math.abs(self.props.coords[t2]-e.originalEvent.touches[0][t2]))
    {
     ...
     if(delta>0)
      self.go({to:self.props.current+self.options.scroll,dir:'forward'});else
      self.go({to:self.props.current-self.options.scroll,dir:'back'});
     e.preventDefault();
     e.stopPropagation();
    }

 ...
 $.extend(Form.prototype,{
  init:function(){
   ...
   self.props=$.extend(true,self.props,{
    ...
    sent:$(self.data.sent),
    captcha:{
     input:self.data.captchaUrl?$(self.data.captcha.input):null,
     img:self.data.captchaUrl?$(self.data.captcha.img):null
    }
   });
  ...
  prepareData:function(){
   ...
   if(inps.filter(':file').length)
   {
    ...
    inps.each(function(){
     ...
     if(obj.is(':file'))
     {
      [].forEach.call(obj[0].files,function(o){
       fd.append(obj.attr('name'),o);
      });
     }else
     {

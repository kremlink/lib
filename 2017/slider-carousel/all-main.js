(function (factory) {
 ...
 mgr.setBlock(function(){
  ...
  var slider=function(opts){
    ...
    ext=opts.data.extra_,
    items=$(opts.data.elements),
    imgs=$();

    for(var i=0;i<items.length;i++)
     imgs=imgs.add($('<img />').attr('src',items.eq(i).data(ext.data)));
    imgs.imagesLoaded(function(){
     mgr.set({data:opts.name,object:'Slider',extra:{helpers:{swipe:mgr.get('lib.utils.swipe')}}});

     cont.addClass(ext.cls);
     items.each(function(){
      var obj=$(this);

      obj.css('background-image','url('+obj.data(ext.data)+')');
     });
     imgs.remove();
    });
   };

...
function Datepicker() {
  this._defaults = {
		...
		showSpeed: 'normal', // Duration of display/closure
		      popupContainer: null, //The element to which a popup calendar is added, null for body---added
        offset:{left:0,top:0},//Offset from target---added
        unselectable:'datepick-unselectable',//Unselectable date yet without selectable:true---added
        noClosingOnSelect:false,//Closes either by clicking outside or by invoking hide method---added
        noChooseOnSelect:false,//Wait for user to make all dates selections and then invoke setDate method---added
        onSelectDate:null,//Called every time date is selected(but, maybe, not set)---added
        onOpen: null,//Called every time datepick is opened---added
		...
		onShow: null, // Callback just before a datepicker is shown (before onOpen and on every redraw)
...
$.extend(Datepicker.prototype, {
  ...
  if ($.isFunction(inst.options.onOpen)) {
   inst.options.onOpen.apply(target, [inst]);
  }
	},
				...
				if (!inst.div) {
					inst.div = $('<div></div>').addClass(this._popupClass).
						css({display: (hidden ? 'none' : 'static'), position: 'absolute',
							left: target.offset().left-$(inst.options.popupContainer || 'body').offset().left+inst.options.offset.left,
							top: target.offset().top + target.outerHeight() - $(inst.options.popupContainer || 'body').offset().top+inst.options.offset.top}).
	...
	_checkOffset: function(inst) {
		...
		return offset;*/
        return {left:inst.div.css('left'),top:inst.div.css('top')};
	},
    ...
    @param  onlyHighlight   (boolean) - added*/
	_setDatePlugin: function(target, dates, endDate, keyUp, setOpt, onlyHighlight) {
		var inst = $.data(target, this.propertyName);
		if (inst) {
			if (!$.isArray(dates)) {
				dates = [dates];
   ...
   if(inst.options.onSelectDate)
    inst.options.onSelectDate(inst,elem);
   if(!inst.options.noChooseOnSelect)
			    this._updateInput(target);
            
			if (inst.options.noClosingOnSelect || inst.inline || inst.pickingRange || inst.selectedDates.length <
	...
	_generateContent: function(target, inst) {
		...
		picker.find(inst.options.renderer.daySelector + ' a').hover(
			...
			click(function() {
    if(!$(this).hasClass(inst.options.unselectable))
		...
		for (var week = 0; week < numWeeks; week++) {
			...
			for (var day = 0; day < 7; day++) {
				...
				var dateInfo = (!$.isFunction(inst.options.onDate) ? {} :
					inst.options.onDate.apply(target, [drawDate, drawDate.getMonth() + 1 == month,inst]));
